...
def generate_booking_id(length=8):
    while True:
        booking_id = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(length))
...
def calculate_total_amount():
    ...
    vehicle_total_amount = vehicle.objects.aggregate(total_amount=models.Sum('total_amount'))['total_amount'] or 0
    
    # Retrieve total_amount from Prebook model
    prebook_total_amount = Prebook.objects.aggregate(total_amount=models.Sum('total_amount'))['total_amount'] or 0
