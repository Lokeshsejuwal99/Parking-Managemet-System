...
class vehicle(models.Model):
    ...
    vehicle_type = models.CharField(
        max_length=10, choices=[("2_Wheels", "2 W"), ("4_Wheels", "4 W")]
    )
        ...
        if is_new_instance or self.Space_to_reserve != self._original_Space_to_reserve:
            if self.Space_to_reserve <= self.parking_lot.available_spaces:
                if not is_new_instance:
                    ...
                    self.parking_lot.total_reserved_spaces -= (
                        self._original_Space_to_reserve
                    )
...
def calculate_total_amount():
    ...
    vehicle_total_amount = (
        vehicle.objects.aggregate(total_amount=Sum("total_amount"))["total_amount"] or 0
    )

    # Retrieve total_amount from Prebook model
    prebook_total_amount = (
        Prebook.objects.aggregate(total_amount=Sum("total_amount"))["total_amount"] or 0
    )
