...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for day_offset in range(7):
            ...
            # Check if a ParkingLot instance already exists for the current
            # date
            if not ParkingLot.objects.filter(date=current_date).exists():
                ParkingLot.objects.create(
                    date=current_date,
                    name=f"Parking Lot {current_date}",
                    total_spaces=500,
                    available_spaces=500,
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully created ParkingLot instance for {current_date}"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"ParkingLot instance for {current_date} already exists"
                    )
                )
