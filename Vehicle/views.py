...
from .forms import vehicleModelform, CustomUserCreationForm, BookingForm, PaymentForm, FeedbackForm
...
def superuser_login(request):
    ...
    return render(request, 'Vehicle/only_users.html', {'error_message': 'Invalid username or password.'})
...
def DashboardView(request):
    ...
    afternoon_vehicles = vehicle.objects.filter(entry_time__time__lt=evening_start).exclude(entry_time__time__lt=morning_start)
    ...
    afternoon_prebooked = Prebook.objects.filter(entry_time__time__lt=evening_start).exclude(entry_time__time__lt=morning_start)
    ...
    morning_count = morning_vehicles.aggregate(Sum('Space_to_reserve'))['Space_to_reserve__sum'] or 0
    afternoon_count = afternoon_vehicles.aggregate(Sum('Space_to_reserve'))['Space_to_reserve__sum'] or 0
    evening_count = evening_vehicles.aggregate(Sum('Space_to_reserve'))['Space_to_reserve__sum'] or 0

    # Aggregate prebooked spaces
    morning_prebooked_count = morning_prebooked.aggregate(Sum('reserved_spaces'))['reserved_spaces__sum'] or 0
    afternoon_prebooked_count = afternoon_prebooked.aggregate(Sum('reserved_spaces'))['reserved_spaces__sum'] or 0
    evening_prebooked_count = evening_prebooked.aggregate(Sum('reserved_spaces'))['reserved_spaces__sum'] or 0
...
def payment_view(request, prebook_id):
    ...
    if request.method == 'POST':
        ...
        form = PaymentForm()
    return render(request, 'payment.html', {'form':form, 'prebook_id': prebook_instance.id})
...
class GeneratePdf(ListView):
    def get(self, request, pk, *args, **kwargs):
        ...
        prebook = Prebook.objects.filter(id=pk).values('booking_id','Full_name', 'reserved_spaces','vehicle_number', 'entry_time', 'exit_time', 'total_amount')
        Vehicle = vehicle.objects.filter(id=pk).values('booking_id','Full_name', 'Space_to_reserve','vehicle_number', 'Vehicle_model', 'entry_time', 'exit_time', 'total_amount')
